...
if auth.status_code == 200:
    ...
    price = megalib.new_vxc_price(auth.header, input('Location ID: '), input('B_End Location ID: '), input('Speed: '),
                                  prod=False)
    ...
    # Advise user if lookup failed, print the status code & JSON
        ...
        print('VXC Price Lookup Failed, Status Code: ' + str(price.status_code) + ', JSON: ' + str(price.json))
